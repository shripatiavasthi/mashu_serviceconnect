...
export default function ServiceData({ navigation, route, ...props }) {
  ...
  useEffect(() => {
    dispatch(getLegalInformationServiceData(name));
  }, []);

  let data = useSelector(({ legalInformationServiceData }) => {
    return legalInformationServiceData.legalInformationServiceData;
  });

  let legalInformationServiceDataFetchInProgress = useSelector(
    ({ legalInformationServiceData }) =>
      legalInformationServiceData.legalInformationServiceDataFetchInProgress,
  );
  let legalInformationServiceDataFetchError = useSelector(
    ({ legalInformationServiceData }) =>
      legalInformationServiceData.legalInformationServiceDataFetchError,
  );

  return (
    <>
      {legalInformationServiceDataFetchInProgress ? (
        <Loader />
      ) : !legalInformationServiceDataFetchError && data ? (
