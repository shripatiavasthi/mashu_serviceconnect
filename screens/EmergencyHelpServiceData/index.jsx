...
import { getEmergencyHelpServiceData } from './store/dispatchers';
...
export default function ServiceData({ navigation, route, ...props }) {
  ...
  useEffect(() => {
    dispatch(getEmergencyHelpServiceData(name));
  }, []);

  let data = useSelector(({ emergencyHelpServiceData }) => {
    return emergencyHelpServiceData.emergencyHelpServiceData;
  });

  let emergencyHelpServiceDataFetchInProgress = useSelector(
    ({ emergencyHelpServiceData }) =>
      emergencyHelpServiceData.emergencyHelpServiceDataFetchInProgress,
  );
  let emergencyHelpServiceDataFetchError = useSelector(
    ({ emergencyHelpServiceData }) =>
      emergencyHelpServiceData.emergencyHelpServiceDataFetchError,
  );

  return (
    <>
      {emergencyHelpServiceDataFetchInProgress ? (
        <Loader />
      ) : !emergencyHelpServiceDataFetchError && data ? (
