...
import {getBuildYourSkills} from './store/dispatchers';
...
export default function BuildYourSkills({navigation, route, ...props}) {
  const dispatch = useDispatch();
  const {colors, fontSize, fontFamily} = useTheme().theme;

  useEffect(() => onRefresh(), []);

  const onRefresh = () => dispatch(getBuildYourSkills());

  let data = useSelector(
    ({buildYourSkills}) => buildYourSkills.buildYourSkills,
  );
  let buildYourSkillsFetchInProgress = useSelector(
    ({buildYourSkills}) => buildYourSkills.buildYourSkillsFetchInProgress,
  );
  let buildYourSkillsFetchError = useSelector(
    ({buildYourSkills}) => buildYourSkills.buildYourSkillsFetchError,
  );

  return (
    <>
      {buildYourSkillsFetchInProgress ? (
        <Loader />
      ) : !buildYourSkillsFetchError && data ? (
        <>
          {data.overview && (
            ...
            renderItem={({item}) => (
              <View style={styles.cardsContainer}>
                <MainCard
                  onPress={() =>
                    navigation.navigate(
                      'buildYourSkillsServiceDataScreen',
                      item,
                    )
                  }
